...
const appAgentLogin = async (req: RequestType, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const doesAccessGroupExist = await appAccessGroupModel.findOne({
...
const appAgentRefresh = async (req: RequestType, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Check if agent exist in Collection
    const getAgent = await appAgentModel.findOne({
      _id: stringToObjectId(req.payload.appAgentId),
      isDeleted: false
    });
    ...
    const JWT_REFRESH_TOKEN_HEADER = GlobalConfig.JWT_REFRESH_TOKEN_HEADER
    console.log(JWT_REFRESH_TOKEN_HEADER);
    ...
    res
      .cookie(JWT_REFRESH_TOKEN_HEADER, newJwtRefreshToken, {
        secure: process.env.NODE_ENV == 'production',
        signed: true,
        httpOnly: true,
        sameSite: true,
...
const getAgentByToken = async (req: RequestType, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const payload: any = JWT.verify(jwt_access_token, JWT_ACCESS_TOKEN_SECRET);
    if (payload?.payloadData?.requestIP !== req.ip) {
      throw httpErrors.Unauthorized('Request not Authorized');
    }
    const app_agentID = payload?.payloadData?.appAgentId;
    const agentDetails = await appAgentModel.findOne({
